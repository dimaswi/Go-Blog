...
export default function PortfolioEdit() {
  ...
  const [formData, setFormData] = useState({
    ...
    category_id: '', status: 'draft', sort_order: 0,
    meta_title: '', meta_description: '', meta_keywords: '', og_image: '',
  });

  useEffect(() => {
    setPageTitle('Edit Portfolio');
    loadData();
  }, [id]);
  ...
  const loadData = async () => {
    try {
      const [portRes, catRes] = await Promise.all([
        ...
        portfolioCategoriesApi.getAll(),
      ]);
      const p = portRes.data.data;
      setFormData({
        title: p.title, slug: p.slug, description: p.description || '', content: p.content || '',
        featured_image: p.featured_image || '', images: p.images || '',
        project_url: p.project_url || '', github_url: p.github_url || '',
        tech_stack: p.tech_stack || '', category_id: p.category_id ? String(p.category_id) : '',
        status: p.status, sort_order: p.sort_order,
        meta_title: p.meta_title || '', meta_description: p.meta_description || '',
        meta_keywords: p.meta_keywords || '', og_image: p.og_image || '',
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await portfoliosApi.update(Number(id), {
        ...formData,
        category_id: formData.category_id ? parseInt(formData.category_id) : null,
        sort_order: Number(formData.sort_order),
      });
      toast({ variant: "success", title: "Success!", description: "Portfolio updated." });
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 p-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Update your portfolio project</p>
          </div>
        </div>
        <div className="flex gap-2">
          <Button type="button" variant="outline" onClick={() => navigate('/portfolios')} className="h-9 text-sm">Cancel</Button>
          <Button type="submit" form="portfolio-edit-form" disabled={loading} className="h-9 text-sm min-w-24">
      ...
      {/* Two-column layout */}
      <form id="portfolio-edit-form" onSubmit={handleSubmit} className="grid grid-cols-1 xl:grid-cols-[1fr_300px] gap-5 items-start">
          ...
          <div className="rounded-lg border bg-card p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input id="project_url" value={formData.project_url}
              ...
              <div className="space-y-2">
                ...
                <Input id="github_url" value={formData.github_url}
          ...
          <div className="rounded-lg border bg-card p-4 space-y-2">
            ...
            {formData.featured_image && (
              <img src={formData.featured_image.startsWith('http') ? formData.featured_image : `${BASE_URL}${formData.featured_image}`}
                alt="" className="w-full aspect-video object-cover rounded-md border" />
            )}
